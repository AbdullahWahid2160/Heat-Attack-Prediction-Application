import React, { Component } from 'react';
import { View, Text, StyleSheet, ScrollView, Image, Modal, ActivityIndicator, TouchableOpacity } from 'react-native';
import * as firebase from 'firebase';

import Card from '../../Components/Card';


export default class HomePage extends Component {

	constructor(props) {
		super(props)
		this.state = {
			FullName: "",
			User: firebase.auth().currentUser.uid,
			FirstTime: false,
			Img: '',
			Attention: "",
			HealthChecked: false,
			Status: null,
			Loading: true,
			WP: false
		};
	}

	componentDidMount() {
		firebase
			.database()
			.ref('DoctorsList/')
			.child(this.state.User)
			.on('value', snapshot => {
				var gender = snapshot.val().Gender;
				var FirstName = (snapshot.val().FirstName);
				var LastName = (snapshot.val().LastName);
				this.setState({
					FullName: FirstName + " " + LastName,
				});
				this.state.Loading = false
				if (gender == "Male" || gender == "Female") {
					this.state.FirstTime = false;
				}
				else {
					this.state.FirstTime = true;
				}
				if (this.state.FirstTime == true) {
					this.setState({
						Attention: "We noticed that You haven't \nset up your profile yet."
					});
				}
				else if (this.state.FirstTime == false) {
					firebase
						.database()
						.ref("WorkPlaces/")
						.on('value', snap => {
							snap.forEach(childsnap => {
								if (childsnap.val().DoctorID == this.state.User) {
									this.state.WP = true;
								}
							})
						})
					if (this.state.WP == true) {
						this.setState({
							Attention: "Check and manange new Appointments."
						})
					}
					else if (this.state.WP == false) {
						this.setState({
							Attention: "We noticed that You haven't \nmanaged your workplaces yet."
						})
					}
				}
			});
	}

	HandleNewRequests = () => {
		this.props.navigation.navigate("NewAppointmentRequests")
	};

	ManageAppointments = () => {
		this.props.navigation.navigate("ScheduledApppointments")
	};

	SetUpProfile = () => {
		this.props.navigation.navigate('DoctorProfile')
	};

	ManageWorkPlaces = () => {
		this.props.navigation.navigate('DoctorWorkPlaces')
	};

	HandleLogOut = () => {
		firebase.auth().signOut();
		this.props.navigation.navigate("InitialPage")
	};

	render() {
		return (
			<ScrollView contentContainerStyle={Styles.Container}>

				<Modal
					visible={this.state.Loading}
					transparent={true}
				>
					<View style={Styles.ModalContainer}>
						<View style={Styles.ModalContent}>
							<Text>
								Please wait...
							</Text>
							<ActivityIndicator color='#cf655b' size='large'></ActivityIndicator>
						</View>
					</View>
				</Modal>

				<Text style={Styles.WelcomeText}>Hi {this.state.FullName}! Welcome to HAP.</Text>

				<View style={Styles.WarningsContainer}>

					<Image
						style={Styles.warning}
						source={require('../../Images/Warning.png')}
					/>
					<Text style={Styles.WarningText}>
						{this.state.Attention}
					</Text>
				</View>

				<View style={Styles.CardsContainer1}>
					<TouchableOpacity onPress={this.HandleNewRequests}>
						<Card>
							<Text style={Styles.ButtonText}>
								New Appointment Requests
							</Text>
						</Card>
					</TouchableOpacity>

					<TouchableOpacity onPress={this.ManageAppointments}>
						<Card>
							<Text style={Styles.ButtonText}>
								Manage Appointments
							</Text>
						</Card>
					</TouchableOpacity>
				</View>

				<View style={Styles.CardsContainer2}>
					<TouchableOpacity onPress={this.SetUpProfile}>
						<Card>
							<Text style={Styles.ButtonText}>
								Profile
							</Text>
						</Card>
					</TouchableOpacity>

					<TouchableOpacity onPress={this.ManageWorkPlaces}>
						<Card>
							<Text style={Styles.ButtonText}>
								Manage Workplaces
							</Text>
						</Card>
					</TouchableOpacity>
				</View>

				<View style={Styles.CardsContainer3}>
					<TouchableOpacity onPress={this.HandleLogOut}>
						<Card>
							<Text style={Styles.ButtonText}>
								Logout
						</Text>
						</Card>
					</TouchableOpacity>
				</View>

			</ScrollView>
		)
	}
}

const Styles = StyleSheet.create({
	ModalContainer: {
		backgroundColor: '#000000aa',
		flex: 1
	},
	ModalContent: {
		backgroundColor: 'rgba(255,255,255,0.9)',
		alignItems: "center",
		justifyContent: "center",
		marginVertical: 80,
		marginHorizontal: 10,
		borderRadius: 10,
		flexGrow: 1
	},
	Container: {
		backgroundColor: '#fff',
		flexGrow: 1,
		alignItems: 'center',
		justifyContent: 'center',
	},
	WelcomeText: {
		marginTop: 70,
		fontSize: 20,
		color: 'black'
	},
	WarningsContainer: {
		flexDirection: 'row',
		marginBottom: 30,
		marginTop: 30,
		alignItems: 'center'
	},
	warning: {
		width: 40,
		height: 40
	},
	WarningText: {
		fontSize: 16
	},
	CardsContainer1: {
		flexGrow: 1,
		flexDirection: "row",
	},
	CardsContainer2: {
		flexGrow: 1,
		flexDirection: "row",
	},
	CardsContainer3: {
		flexGrow: 1,
		flexDirection: "row",
		marginBottom: 70
	},
	ButtonText: {
		fontSize: 16,
		fontWeight: '500',
		color: '#cf655b',
		textAlign: 'center',
		paddingBottom: 5
	},
});